...
export const getSneakers = createAsyncThunk(
  ...
  async (settings: TypeUrlSettings) => {
    let res;
    let url: string;
    if (settings.category && settings.filter) {
      url = `https://6528113c931d71583df1d45b.mockapi.io/products?page=${settings.page}&limit=8${
        settings.filter
      }${settings.search ? `&search=${settings.search}` : ''}`;
      if (settings.category !== 'all') {
        url = `https://6528113c931d71583df1d45b.mockapi.io/products?${
          settings.category === 'all' ? '' : `&category=${settings.category}`
        }&page=${settings.page}&limit=8${settings.filter}${
          settings.search ? `&search=${settings.search}` : ''
        }`;
      }
      res = await axios(url);
